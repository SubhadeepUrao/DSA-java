...
public class Optimal2 {
    ...
    public static void solve(List<List<String>> list, boolean[][] board, int col, boolean[] leftRow, boolean[] lowerDiagonal, boolean[] upperDiagonal) {
        int n = board.length;
        if(col == n) {
        ...
        for(int row = 0; row < n; ++row) {
            if(!leftRow[row] && !lowerDiagonal[row+col] && !upperDiagonal[n-1+col-row] && insertValid(board, row, col)) {
                leftRow[row] = true;
                lowerDiagonal[row+col] = true;
                upperDiagonal[n-1+col-row] = true;
                board[row][col] = true;

                solve(list, board, col+1, leftRow, lowerDiagonal, upperDiagonal);
    ...
    public static List<List<String>> solveNQueens(int n) {
        ...
        solve(list, board, 0, leftRow, lowerDiagonal, upperDiagonal);
        return list;
    }
